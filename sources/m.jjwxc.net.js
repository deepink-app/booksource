  ...
  const chapter = (url) => {
      ...
      let response = GET(url)
      if(url.match(/app-cdn/)) return JSON.parse(response).content
      ...
      let content =""
        $('ul > li:has(br)').forEach((child) => {
content  += child.replace(/class=\"\">/g,"")    
  })
 return content     
  }
  ...
  const profile = () => {
      ...
      return JSON.stringify({
          ...
          nickname: decodeURI(decodeURI(COOKIE("nickname"))),
          recharge: 'https://m.jjwxc.net/pay',
          balance: [{
                  name: '月石',
                  coin: $.coinstotal
              },
